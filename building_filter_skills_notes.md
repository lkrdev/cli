...
The filter expressions on this page are also supported for some filter types in Data Studio when the Looker connector references a Looker Explore as a data source and the filter employs a Matches (advanced) condition. The Matches (advanced) condition behaves somewhat differently in Data Studio than it does in a Looker Explore. Looker may convert the matches (advanced) condition to a more appropriate condition, based on the user's input. However, Data Studio uses the Matches (advanced) option as a catch-all condition when the user's input is inappropriate for the other filter conditions that Data Studio supports, but Data Studio doesn't convert the condition.
...
Note: In some cases you may also want to escape or include special characters in string filters when using custom drills to Looker content. See the Things to consider when linking to Looker content section on the Link parameter page for more information.
...
2018-05-18 14:00:00 The initial time value is inclusive but the latter time value is not. So the expression 2018-05-18 12:00:00 to 2018-05-18 14:00:00 will return data with the time "2018-05-18 12:00:00" through "2018-05-18 13:59:59".
...
Note: If your Looker developers have specified using a fiscal year then you can type fiscal in these expressions to use a fiscal year instead of a calendar quarter. For example, you can use last fiscal year.
...
Note: The syntax for numeric filter expressions using NOT may not be intuitive. If the first filter condition contains a NOT, and no other filter conditions contains a NOT, then all of the filter conditions will be negated. See the following examples for more information.
Example Description
5 is exactly 5
NOT 5
<>5
...
For example, suppose you need to apply an sf\_ prefix to the value of the salesforce_username user attribute because that is how the values are stored in your database. To add the prefix to the user attribute value, you can add a matches (advanced) filter on the relevant field and use the \_user_attributes Liquid variable in the filter expression as follows:

sf\_{{_user_attributes['salesforce_username']}}
